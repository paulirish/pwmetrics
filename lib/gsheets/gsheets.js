...
function authenticate(clientSecret) {
  return new Promise((resolve, reject) => {
    try {
      if (process.env.CI)
        return authorizeWithServiceAccount(resolve);
      else
        return authorize(clientSecret, resolve);
    } catch(err) {
      return reject(err);
    }
  });
...
/**
 * Reads service account credentials and instantiates a client with those.
 *
 * @param {function} callback The callback to call with the authorized client.
 */
function authorizeWithServiceAccount(callback) {
  const authFactory = new GoogleAuth();

  if (!process.env.sheets_secret_json)
    return new Error('sheets_secret_json ENV variable is not set.')
  
  const filepath = path.join(process.cwd(), 'sheets_secret.json');
  fs.writeFileSync(filepath, process.env.sheets_secret_json, 'utf-8');
  process.env.GOOGLE_APPLICATION_CREDENTIALS = filepath;
  
  // This will read process.env.GOOGLE_APPLICATION_CREDENTIALS 
  // and interpret as filepath of the oauth service account secret json
