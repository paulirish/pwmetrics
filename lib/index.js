...
const {shareWithTimelineViewer} = require('./timeline-viewer');
...
class PWMetrics {
  ...
  recordLighthouseTrace(data) {
    try {
      ...
      if (this.flags.shareWithTimelineViewer) {
        try {
          return shareWithTimelineViewer(data, this.sheets.options.clientSecret).then(_ => preparedData);
        } catch(e) {
          Promise.reject(e);
        }
      }
