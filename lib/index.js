...
class PWMetrics {
  ...
  appendResults(results) {
    ...
    results.forEach(data => {
      ...
      // const getTiming = key => metricsDefinitions.find(def => def.id === key).getTs(data);
      ...
      valuesToAppend.push([
        ...
        getTiming(metrics.ids.TTFCP),
        getTiming(metrics.ids.TTFMP),
        getTiming(metrics.ids.PSI),
        getTiming(metrics.ids.FV),
        getTiming(metrics.ids.VC100),
        getTiming(metrics.ids.TTI),
        getTiming(metrics.ids.VC85)
      ]);
    });
