...
class PWMetrics {
  ...
  constructor(url, opts) {
    ...
    this.desiredMetrics = [];
    if (this.flags.desiredMetrics) {
      this.desiredMetrics = this.flags.desiredMetrics.filter(metric => {
        if (!Object.values(metrics.ids).includes(metric)) {
          console.log(getMessageWithPrefix('WARNING', 'UNKOWN_METRIC', metric));
          return false;
        }
        return true;
      });
    }
    this.desiredMetrics = this.desiredMetrics.length ? this.desiredMetrics : ['tti', 'psi', 'ttfmp', 'ttfcp'];
  ...
  start() {
    ...
    return allDone.then(_ => {
      ...
      if (this.runs > 1 && !this.flags.submit && ret.runs.length) {
        console.log(getMessage('MEDIAN_RUN'));
        ret.median = this.findMedianRun(ret.runs);
      ...
      } else if (ret.runs.length === 1) {
        this.displayOutput(ret.runs[0]);
      }
      return ret;
    });
  }
  ...
  recordLighthouseTrace(data) {
    try {
      const preparedData = metrics.prepareData(data, this.desiredMetrics);
  ...
  findMedianRun(results) {
    const maxTimingValue = Math.max(...results.reduce((acc, r) => {
      // Result may have invalid data -> failed run
      if (r.timings)
        return acc.concat(r.timings.map(t => t.timing));
      else return acc;
    }, []));

    const largestMetric = results.reduce((obj, result) => {
      const index = result.timings.findIndex(timing => timing.timing === maxTimingValue);
      if (index !== -1) obj = result.timings[index];
      return obj;
    }, {});

    const metricValues = results.map(r => r.timings.find(timing => timing.id === largestMetric.id).timing);
    const medianTTI = median(metricValues);
    ...
    const medianRun = results.find(result => result.timings.find(timing => {
      return timing.id === largestMetric.id && timing.timing === medianTTI;
    }));
    return medianRun;
  }
