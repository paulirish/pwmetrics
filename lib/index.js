...
class PWMetrics {
  ...
  recordLighthouseTrace() {
    ...
    return lighthouse(this.url, lhOpts, perfConfig)
      ...
      .then(data => {
        if (this.opts.sheet) {
          return this.appendResults(data);
        } else {
  ...
  appendResults(data) {
    ...
    console.log(this.sheet.options);
    return gsheets.authenticate().then(auth =>
      gsheets.appendResults(auth, valuesToAppend, this.sheet.options)
    ).catch(e => {
      console.error(e);
    })
  }
